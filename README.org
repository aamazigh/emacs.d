...
*** Hooks
    ...
    #+END_SRC
* Updating README.org from config file

  Automatically update README.org when saving configuration file

  #+BEGIN_SRC emacs-lisp
    (defun update-configuration-file()
    "Write the changes to the configuration file and update README.org"
    (interactive)
    (save-buffer)
    (write-region (point-min) (point-max) "~/.emacs.d/README.org" nil nil nil))
  #+END_SRC
